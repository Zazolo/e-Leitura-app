...
var VerHistoriaPage = /** @class */ (function () {
    function VerHistoriaPage(route, historiaService, auth) {
        this.route = route;
    ...
    VerHistoriaPage.prototype.obterHistoria = function () {
        var _this = this;
        this.zone.run(function () {
            _this.historia = _this.historiaService.getLast();
            console.log("Percorrendo parágrafos...");
            console.log(_this.historia.paragrafos);
        });
    };
    VerHistoriaPage.prototype.ngOnInit = function () {
        var _this = this;
        setInterval(function () {
            _this.usuarioLogadoLogin = _this.auth.getUser();
            console.log(_this.usuarioLogadoLogin);
            _this.obterHistoria();
        }, 3000);
    };
    VerHistoriaPage.prototype.criarParagrafo = function () {
        this.route.navigate(['/criar-paragrafo']);
    };
    VerHistoriaPage.prototype.votar = function (id) {
        var _this = this;
        this.zone.run(function () {
            var i = 0;
            for (i = 0; i < _this.historia.paragrafos_votacao.length; i++) {
                if (_this.historia.paragrafos[i].id == id) {
                    _this.historia.paragrafos_votacao[i] = tslib__WEBPACK_IMPORTED_MODULE_0__["__assign"]({}, _this.historia.paragrafos_votacao[i], { votou: true });
                }
                else {
                    _this.historia.paragrafos_votacao[i] = tslib__WEBPACK_IMPORTED_MODULE_0__["__assign"]({}, _this.historia.paragrafos_votacao[i], { votou: false });
                }
            }
            _this.historiaService.votar(id).then(function (ok) {
    ...
    VerHistoriaPage.prototype.removerParagrafo = function (id) {
        var _this = this;
        this.zone.run(function () {
            var i = 0;
            var paragrafo = null;
            for (i = 0; i < _this.historia.paragrafos_votacao.length; i++) {
                if (_this.historia.paragrafos_votacao[i].id == id) {
                    paragrafo = _this.historia.paragrafos_votacao[i];
                    _this.historia.paragrafos_votacao.splice(i, 1);
                }
            }
            _this.historiaService.removerParagrafo(id).then(function (ok) {
    ...
    VerHistoriaPage.prototype.finalizarHistoria = function () {
        var _this = this;
        this.historiaService.finalizarHistoria(this.historia.id).then(function (finalizou) {
            alert("Historia finalizada com sucesso!");
            _this.route.navigate(['tabs', 'historia']);
        }).catch(function (error) {
            alert("Erro ao finalizar a historia!");
        });
    };
    VerHistoriaPage.ctorParameters = function () { return [
